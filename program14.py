...
for i in digit:
    ...
    if bit < 99:
        ...
        bit_score.append(int(x))

    else:
        bit_score.append(int((str(bit)[1:])[0]))## or use bit_score.append(bit%100)
...
for i in range(len(bit_score)):
    for j in range(i+1,len(bit_score)):
        if bit_score[i] == bit_score[j]:
