...
for _ in range(t):
    ...
    x = 65
    for i in range(N):
        original_position.append(chr(x))
        x+=1
    initial = "".join(original_position)
    for i in range(10000000):
        temp = "".join(original_position)
        for j in range(N):
            temp = list(temp)
            original_position[monkeys[j]] = temp[j]
        if "".join(original_position) == initial:
            print(i+1)
            break
