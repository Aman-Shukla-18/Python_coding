...
'''
contestents = int(input())
brides = list(input())
grooms = list(input())
match = 0

for i in range(contestents):

    x = match
    for j in range(i , contestents):
        if brides[i] == grooms[i]:
            match += 1
            break
        else:
            grooms.append(grooms.pop(i))

    if match != x+1:
        break
unmatch = contestents - match
print(unmatch)  ## number of unmatched pairs
